...
def write_lammps_input_langevin(filament_name: filament, box_dimensions: list, mass: list, bond_styles: list, angle_styles: list, pair_coeff: list, pair_cutoffs: list, groups: list, sim_parameters: list, folders: list, langevin_parameters: list, input_fname_str: str, filament_datafile: str, dump_minimization: bool, fix_nve_min: list, fix_nve_run: list, fix_wall: list):
    ...
    temperature_min, temperature_max = temperture

    with open(input_fname_str, 'w') as input_f:

        input_f.write(
            "# LAMMPS input script for trapezoidal filament with flat linker\n\n")
        ...
        input_f.write(
            "variable record_interval equal {:d}\n".format(record_interval))
        input_f.write(
            "variable dump_interval_min equal {:d}\n".format(dump_interval_min))
        input_f.write(
            "variable dump_interval_run equal {:d}\n".format(dump_interval_run))
        ...
        input_f.write(
            "variable langevin_seed equal {:d}\n".format(langevin_seed))
        ...
        input_f.write("region membrane cylinder {} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f}\n".format(
            cyl_dim, cyl_c1, cyl_c2, cyl_rad, cyl_lo, cyl_hi))
        ...
        for coeff_val in pair_coeff:
            ...
            if pair_style == "zero":
                ...
                input_f.write("pair_coeff {} {} {} {:.4f} {:.4f} {:.4f}\n".format(
                    i1, i2, pair_style, epsilon, sigma, Rc))
            else:
        ...
        if dump_minimization:
            input_f.write(
                "dump minimization all atom ${dump_interval_min} dump/dump.min.${xx}.lammpstrj\n")
            input_f.write("\n")
        ...
        for fix_wall_i in fix_wall:
            ...
            input_f.write("fix {} {} wall/region membrane lj93 {:.4f} {:.4f} {:.4f}\n".format(
                fix_name, fix_type, epsilon, sigma, cutoff))
        ...
        input_f.write(
            "dump dumpall all atom ${dump_interval_run} dump/dump.${xx}.lammpstrj\n")
        input_f.write("\n")
        ...
        for linker_i, linker in enumerate(filament_name.linkers):
            ...
            for atom_i in range(len(linker.positions)):
                ...
                input_f.write("variable l{}a{}x equal x[{}]\n".format(
                    linker_i+1, atom_i+1, atom_index))
            ...
            input_f.write("variable l{}x equal (v_l{}a1x+v_l{}a2x+v_l{}a3x+v_l{}a4x)/4\n".format(
                linker_i+1, linker_i+1, linker_i+1, linker_i+1, linker_i+1))
        ...
        input_f.write(
            "fix printlinkers all print ${record_interval} \"${tsteps} ")
        for linker_i, linker in enumerate(filament_name.linkers):
            input_f.write("${{l{}x}} ${{l{}y}} ${{l{}z}} ".format(
                linker_i+1, linker_i+1, linker_i+1))
        ...
        input_f.write(
            "fix printlinkerex all print ${record_interval} \"${tsteps} ")
        for linker_i, linker in enumerate(filament_name.linkers_2):
            input_f.write("${{le{}x}} ${{le{}y}} ${{le{}z}} ".format(
                linker_i+1, linker_i+1, linker_i+1))
        ...
        for layer_i in range(num_layers - 1):
            ...
            for atom_i in range(len(layer_1.positions)):
                ...
                input_f.write("variable m{}a{}x equal x[{}]\n".format(
                    monomer_counter, atom_i+1, atom_index))
            ...
            for atom_i in range(len(layer_2.positions)):
                ...
                input_f.write("variable m{}a{}x equal x[{}]\n".format(
                    monomer_counter, atom_i+1+len(layer_1.positions), atom_index))
                input_f.write("variable m{}a{}y equal y[{}]\n".format(
                    monomer_counter, atom_i+1+len(layer_1.positions), atom_index))
                input_f.write("variable m{}a{}z equal z[{}]\n".format(
                    monomer_counter, atom_i+1+len(layer_1.positions), atom_index))
            ...
            input_f.write("variable m{}x equal (v_m{}a1x+v_m{}a2x+v_m{}a3x+v_m{}a4x+v_m{}a5x+v_m{}a6x+v_m{}a7x+v_m{}a8x)/8\n".format(monomer_counter,
                          monomer_counter, monomer_counter, monomer_counter, monomer_counter, monomer_counter, monomer_counter, monomer_counter, monomer_counter))
        ...
        input_f.write(
            "fix printmonomers all print ${record_interval} \"${tsteps} ")
        for monomer_i in range(1, num_monomers + 1):
            input_f.write("${{m{}x}} ${{m{}y}} ${{m{}z}} ".format(
                monomer_i, monomer_i, monomer_i))
