...
def save_filament_info(filament_name: filament, info_file_name: str):
    ...
    with open(info_file_name, 'w') as info_file:
        info_file.write("#R \t d \t d_e \t a \t a1 \t a2 \t l \t s1 \t s2 \t aF \t aL \t theta1 \t theta2 \t gamma \t phi1 \t phi2 \t phi3 \t phi4 num_monomers \t num_layers \t num_total_particles \t num_linkers \t num_bonds \t num_angles\n")
        
        info_file.write("{:.4f} \t".format(R))
        info_file.write("{:.4f} \t".format(d))
        info_file.write("{:.4f} \t".format(d_e))
